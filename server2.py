...
def download_brdc_from_bkg(obs_date: date, out_dir: str) -> str:
    ...
    try:
        ...
        print(f"[СЕРВЕР] ОШИБКА при скачивании эфемерид: {e}")
...
def run_rtklib_kinematic(rover_file: str, base_file: str, nav_file: str, out_file: str):
    ...
    cmd = ["/Users/sergeidolin/RTKLIB/app/consapp/rnx2rtkp/gcc/rnx2rtkp", "-p", "3", "-o", out_file, rover_file, base_file,  nav_file]
...
def handle_client(conn):
    ...
    rover_path = base_path = eph_path = result_path = None
    try:
        # Получаем количество файлов (должно быть 2)
        num_files = struct.unpack('>I', recv_exactly(conn, 4))[0]
        if num_files != 2:
            raise RuntimeError(f"Ожидается 2 файла, получено {num_files}")

        received_files = []
        # Получаем два файла наблюдений
        for i in range(num_files):
         ...
         # Сохраняем RINEX во временный файл
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as f:
                ...
                print(f"[Сервер] Получен файл {i+1}: {filename} -> {file_path}")

        rover_path, base_path = received_files

        # Извлекаем дату
        obs_date = extract_date_from_rinex(rover_path)
        ...
        for p in [rover_path, base_path, eph_path, result_path]:
